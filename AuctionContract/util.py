...
# call application from contract
def call_app_from_contract(client, private_key, index, contract, method_name="increment", method_args=[]):
    # get sender address
    sender = account.address_from_private_key(private_key)
    ...
    atc.add_method_call(
        app_id=index,
        method=get_method(method_name,contract),
        sender=sender,
        sp=sp,
        signer=signer,
