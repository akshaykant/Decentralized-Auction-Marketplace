# Description of demo 0:
#  This script demonstrates a case of a Vickrey style auction with deposit.
#  Two bidders compete for a NFT. All create valid bids - i.e. above the asking price.
#  All bidders reveal their bids during the reveal phase and get their deposits refunded.
#  The one with the highest bid amount wins.
#  The winner opts in the NFT and claims it.
#  The winner gets refunded up to the second bid since this is Vickrey style auction.
...
from AuctionSealedUtils import *
...
BID_AMOUNTS = [200_000, 350_000]

# Deposit required to be able to participate in the auction
DEPOSIT = 100_000

# Increment = minimal increase in price to consider it valid
INCREMENT = 10_000
...
def main():
    ...
    app_id, contract = createAuctionApp(algod_client, creator_private_key,
                                        ...
                                        endRound, RESERVE, INCREMENT, DEPOSIT, AUCTION_TYPE, SERVICE_FEE)
    ...
    for i, bsk in enumerate(bidders_sk):
        commitAuctionApp(algod_client, app_id, bsk, BID_AMOUNTS[i], NONCES[i], DEPOSIT)
